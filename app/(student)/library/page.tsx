...
export default async function LibraryPage({ searchParams }: LibraryPageProps) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Заход C.6: строка обещала «персональную сессию просмотра с водяным
          знаком» и запрет скачивания — ни того, ни другого нет: плеера у записи
          нет, она открывается ссылкой на чужой вкладке (заход C.4/C.5, где ту же
          неправду уже сняли со страницы записи, а здесь не заметили). */}
      <p className="text-text-3 text-[12px]">
        Записи доступны лично тебе: каждое открытие фиксируется за твоим аккаунтом.
      </p>
    </div>
  );
}
