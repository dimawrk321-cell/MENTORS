...
/**
 * How strong the name match is. `matchCategoryName` tries exact-normalized, then
 * parens-stripped, then prefix — we re-derive which one fired so a reviewer can
 * trust the top rows and scrutinise the bottom ones.
 */
function confidenceOf(target: string, categoryTitle: string): MatchConfidence {
...
export async function suggestLessonCategories(db: Db): Promise<LessonSuggestion[]> {
  ...
  for (const lesson of lessons) {
    ...
    const consider = (target: string, matchedOn: "урок" | "модуль") => {
      const hit = matchCategoryName(target, categories);
      if (!hit || seen.has(hit.id)) return;
      seen.add(hit.id);
      suggestions.push({
        categoryId: hit.id,
        categoryTitle: hit.title,
        categoryPath: pathOf(hit, byId),
        questionCount: countByCategory.get(hit.id) ?? 0,
        confidence: confidenceOf(target, hit.title),
        matchedOn,
        alreadyLinked: linkedByCategory.get(hit.id) ?? 0,
      });
    };

    consider(lesson.title, "урок");
    // The module title is a broader, weaker signal — only when the lesson's own
    // title matched nothing, or the whole module would be tagged twice over.
    if (suggestions.length === 0) consider(lesson.module.title, "модуль");
